...
async def websocket_test_endpoint(
    ...
    await manager1.connect(websocket, chatting_room_id, user_id)
    try:
        for t_websocket in manager1.active_connections[chatting_room_id]:
            t_user_id = manager1.active_connections[chatting_room_id][t_websocket][
                "user_id"
            ]
            if t_user_id != user_id:
                await manager1.send_personal_message(
        ...
        manager1.disconnect(websocket, chatting_room_id, user_id)
        await manager1.broadcast(
...
def chat_html1(user_id, chat_id):
    html = f"""
    <!DOCTYPE html>
    <html>
        <head>
...
@router.get("/test2/{chat_id}/{user_id}")
async def test(chat_id: int, user_id: int):
    print(manager.active_connections)
    return HTMLResponse(chat_html1(user_id, chat_id))
